...
def get_args():
    ...
    parser.add_argument("--frame_rate", type=int, default=2, help="The frame rate of loading input video. Default rate is computed according to video length.")
    parser.add_argument("--fps", type=int, default=15, help="FPS of the output video")
    ##Add Spynet
    parser.add_argument("--spynet", action='store_true', help="Use Spynet, otherwise use Raft")
...
if __name__ == "__main__":
    ...
    trajectories = sample_trajectories(os.path.join(args.output_path, "source_video.mp4"), device, spynet_flag)
    ...
    if spynet_flag == 0:
        if args.neg_prompt == " ":
            temp_video_name = "raft_" + args.prompt+"_None_"+str(args.guidance_scale)
        else: temp_video_name = "raft_" + args.prompt+"_"+args.neg_prompt+"_"+str(args.guidance_scale)
    elif spynet_flag == 1:
        if args.neg_prompt == " ":
            temp_video_name = "spynet_" + args.prompt+"_None_"+str(args.guidance_scale)
        else: temp_video_name = "spynet_" + args.prompt+"_"+args.neg_prompt+"_"+str(args.guidance_scale)
    save_videos_grid(sample, f"{args.output_path}/{temp_video_name}_{args.video_length}_{args.frame_rate}.mp4", fps=args.fps)
