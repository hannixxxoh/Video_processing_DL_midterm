...
from .spynet_flatten import estimate
...
@torch.no_grad()
def sample_trajectories(video_path, device, spynet_flag):
    ...
    current_frames, next_frames = preprocess(frames[clips[:-1]], frames[clips[1:]], transforms) # torch.Size([14, 3, 512, 512])

    print("flag: "+ str(spynet_flag))
    if spynet_flag == 1:
        # Spynet
        print("Using Spynet for optical flow")
        spynet_output_list = []
        for i in range(len(current_frames)):
            spynet_output_list.append(estimate(current_frames[i], next_frames[i]))

        spynet_output_tensor = torch.stack(spynet_output_list) # Still in CPU
        print(spynet_output_tensor.shape)
        predicted_flows = spynet_output_tensor.to(device)

        ###################### Print Optical Flow ##########################
        # spynet_output_img = flow_to_image(spynet_output_tensor[0])/255.0 #
        # save_image(spynet_output_img, './spynet_flow.jpg')               #
        ####################################################################
    elif spynet_flag == 0:
        # Raft
        print("Using Raft for optical flow")
        list_of_flows = model(current_frames.to(device), next_frames.to(device))
        print(list_of_flows[-1].shape)
        predicted_flows = list_of_flows[-1] # In Cuda

        ###################### Print Optical Flow ##########################
        # raft_output_img = flow_to_image(predicted_flows[0].cpu())/255.0  #
        # save_image(raft_output_img, './raft_flow.jpg')                   #
        ####################################################################
